...
func init() {
	...
	// Generate minikube specific directory
	folder := filepath.Join(".", "docs", minikube.Name)
	os.Mkdir(folder, os.ModePerm)
	// Generate versions.json
	toolVersions, err := getMinikubeVersions()
	if err != nil {
		log.Fatal(err)
	}
	...
	template := generateMinikubeVersion("{{.Version}}")
	err = writeJson(folder, "template.json", template)
	if err != nil {
		log.Fatal(err)
	}
	// Generate latest.json
	latest := generateMinikubeVersion(minikube.Latest)
